    ...
    *((uint32_t *)(&(bufFIL[0]))) = *((uint32_t *)(&(bufSCR[265]))); //copy trigger enable channels from SCR to FIL tregister
    ...
    if (feb == BoardToMon)
    {
      for (int i = 265; i < 265 + 32; i++)
        if (ConfigGetBit(bufSCR, 1144, i))
          fChanEnaTrig[i - 265]->SetOn();
        else
          fChanEnaTrig[i - 265]->SetOn(kFALSE);

      if (ConfigGetBit(bufSCR, 1144, 1139))
        fChanEnaTrig[32]->SetOn();
      else
        fChanEnaTrig[32]->SetOn(kFALSE);

      for (int i = 0; i < 32; i++)
        if (ConfigGetBit(bufSCR, 1144, 633 + i * 15))
      ...
      for (int i = 0; i < 32; i++)
        if (ConfigGetBit(bufPMR, 224, 96 + i))
          fChanProbe[i]->SetOn();
        else
          fChanProbe[i]->SetOn(kFALSE);
...
UChar_t ConfigGetGain(int chan)
{
  ...
  {
    val = val << 1;
    if (ConfigGetBit(bufSCR, 1144, 619 + chan * 15 + b))
      val = val + 1;
  }
  return val;
}
...
UChar_t ConfigGetBias(int chan)
{
  ...
  {
    val = val << 1;
    if (ConfigGetBit(bufSCR, 1144, 331 + chan * 9 + b))
      val = val + 1;
  }
  return val;
}
...
void ConfigSetGain(int chan, UChar_t val)
{
  ...
  {
    if ((val & mask) > 0)
      ConfigSetBit(bufSCR, 1144, 619 + chan * 15 + b, kTRUE);
    else
      ConfigSetBit(bufSCR, 1144, 619 + chan * 15 + b, kFALSE);
    mask = mask >> 1;
  }
}
...
void ConfigSetBias(int chan, UChar_t val)
{
  ...
  {
    if ((val & mask) > 0)
      ConfigSetBit(bufSCR, 1144, 331 + chan * 9 + b, kTRUE);
    else
      ConfigSetBit(bufSCR, 1144, 331 + chan * 9 + b, kFALSE);
    mask = mask >> 1;
  }
}
...
void SendConfig()
{
  ...
  for (int i = 265; i < 265 + 32; i++)
    if (fChanEnaTrig[i - 265]->IsOn())
      ConfigSetBit(bufSCR, 1144, i, 1);
    else
      ConfigSetBit(bufSCR, 1144, i, 0);

  if (fChanEnaTrig[32]->IsOn())
    ConfigSetBit(bufSCR, 1144, 1139, 1);
  else
    ConfigSetBit(bufSCR, 1144, 1139, 0); //OR32 enable

  for (int i = 0; i < 32; i++)
    if (fChanEnaAmp[i]->IsOn())
      ConfigSetBit(bufSCR, 1144, 633 + i * 15, 0);
    else
      ConfigSetBit(bufSCR, 1144, 633 + i * 15, 1);

  for (int i = 0; i < 32; i++)
    if (fChanProbe[i]->IsOn())
      ConfigSetBit(bufPMR, 224, 96 + i, 1);
    else
      ConfigSetBit(bufPMR, 224, 96 + i, 0);

  for (int i = 0; i < 32; i++)
    ConfigSetBias(i, fChanBias[i]->GetNumber());
  for (int i = 0; i < 32; i++)
    ConfigSetGain(i, fChanGain[i]->GetNumber());

  for (int i = 0; i < 32; i++)
    if (fChanEnaTrig[i]->IsOn())
      trigmask = trigmask | (0x1 << i);
...
void RescanNet()
{
  ...
  for (int feb = 0; feb < t->nclients; feb++)
  {
    ts0_ref_AVE[t->macs[feb][5]] = 0;
    ts0_ref_IND[t->macs[feb][5]] = 0;
  }
  UpdateConfig();
  fNumberEntry8869->SetLimitValues(0, t->nclients - 1);
  for (int feb = 0; feb < t->nclients; feb++)
...
void FillHistos(int truncat) // hook called by libFEBDTP when event is received
{
  ...
  while (jj < truncat - 18)
  {
    ...
    jj = jj + 2;
    ts0 = *(UInt_t *)(&(t->gpkt).Data[jj]);
    jj = jj + 4;
    ts1 = *(UInt_t *)(&(t->gpkt).Data[jj]);
    jj = jj + 4;
    ...
    if (ts1 != 0)
      gts1[mac5]->SetPoint(gts1[mac5]->GetN(), gts1[mac5]->GetN(), ts1);
    tr->Fill();
    if (t->dstmac[5] == t->macs[BoardToMon][5])
    {
      evs++;
...
void DAQ(int nev = 0)
{
  ...
  while (RunOn == 1 && (nevv == 0 || evs < nevv) && (fNumberEntryTME->GetNumber() == 0 || tm1 - tm0 < fNumberEntryTME->GetNumber()))
  {
    ...
    if (rate < 3.6)
      fStatusBar739->GetBarPart(0)->SetBackgroundColor(0xbbffbb);
    else if (rate < 5.0)
      fStatusBar739->GetBarPart(0)->SetBackgroundColor(0xffbbbb);
    else if (rate < 7.0)
      fStatusBar739->GetBarPart(0)->SetBackgroundColor(0xff9999);
    else if (rate < 10.0)
      fStatusBar739->GetBarPart(0)->SetBackgroundColor(0xff7777);
    else
      fStatusBar739->GetBarPart(0)->SetBackgroundColor(0xff3333);
    ...
    if (nevv > 0)
      printf("%d events to go...\n", nevv - evs);
    sprintf(str1, "Poll: %d events acquired in %2.2f sec.", evsperrequest, PollPeriod);
    if (evsperrequest > 0)
      fStatusBar739->GetBarPart(1)->SetBackgroundColor(0xbbffbb);
    else
      fStatusBar739->GetBarPart(1)->SetBackgroundColor(0xffaaaa);

    fStatusBar739->SetText(str1, 1);
    sprintf(str1, "Evs lost FPGA:%d CPU:%d ", lostinfpga, overwritten);
    if (overwritten == 0)
      fStatusBar739->GetBarPart(2)->SetBackgroundColor(0xbbffbb);
    else if (overwritten < 10)
      fStatusBar739->GetBarPart(2)->SetBackgroundColor(0xffbbbb);
    else if (overwritten < 100)
      fStatusBar739->GetBarPart(2)->SetBackgroundColor(0xff9999);
    else if (overwritten < 1000)
      fStatusBar739->GetBarPart(2)->SetBackgroundColor(0xff7777);
    else
      fStatusBar739->GetBarPart(2)->SetBackgroundColor(0xff3333);
    fStatusBar739->SetText(str1, 2);
    if (NOts0_mon)
    {
      sprintf(str1, "PPS missing!");
      fStatusBar739->GetBarPart(3)->SetBackgroundColor(0xffaaaa);
    }
    else
    {
      sprintf(str1, "PPS period %d ns", ts0_ref_mon);
      fStatusBar739->GetBarPart(3)->SetBackgroundColor(0xbbffbb);
    }
    fStatusBar739->SetText(str1, 3);
    if (NOts1_mon)
    {
      sprintf(str1, "SPILL trig missing!");
      fStatusBar739->GetBarPart(4)->SetBackgroundColor(0xffaaaa);
    }
    else
    {
      sprintf(str1, "SPILL trig period %d ns", ts1_ref_mon);
      fStatusBar739->GetBarPart(4)->SetBackgroundColor(0xbbffbb);
    }
...
void Reset()
{
  ...
  for (int feb = 0; feb < t->nclients; feb++)
  {
    gts0[t->macs[feb][5]]->Set(0);
  }
  for (int feb = 0; feb < t->nclients; feb++)
  {
    gts1[t->macs[feb][5]]->Set(0);
  }
  c3->cd(1);
  //    hcprof->Draw("hist");
  c3->Update();
...
void SetThresholdDAC1(UShort_t dac1)
{
  ...
  {
    if ((dac1 & 1) > 0)
      ConfigSetBit(bufSCR, 1144, offset + 9 - i, kTRUE);
    else
      ConfigSetBit(bufSCR, 1144, offset + 9 - i, kFALSE);
    dac1 = dac1 >> 1;
  }
...
UShort_t GetThresholdDAC1()
{
  ...
  {
    dac1 = dac1 >> 1;
    if (ConfigGetBit(bufSCR, 1144, offset + 9 - i))
      dac1 = dac1 | 0x0200;
  }
  return dac1;
}
...
bool ProgramFW(uint32_t startaddr = 0, uint16_t blocks = 1)
{
  ...
  if (t->SendCMD(t->dstmac, FEB_WR_FW, 00, buf) == 0)
    retval = 0; //initiate flash programming
  for (int i = 0; i < blocks; i++)
  {
    if (t->SendCMD(t->dstmac, FEB_DATA_FW, getCRC(fwbuf + i * 1024, 1024), fwbuf + i * 1024) == 0)
      retval = 0;
  }
  if (retval == 0)
    printf("Error in ProgramFW!!\n");
  return retval;
}
...
bool UpdateFW(char *fname = NULL)
{
  ...
  buf[2] = 0x02;
  buf[3] = 64;
  buf[4] = 0;                                    //prepare for copying 64 kB from 0x20000 to 0x0
...
bool UpdateFPGA(char *fname = NULL)
{
  ...
  for (int page64 = 0; page64 < 8; page64++)
  {
    ...
    if (retval == 0)
      printf("Error in programming 64kB block at %08x !\n", startaddr);
    startaddr = startaddr + 64 * 1024;
  }
  if (retval == 0)
    printf("Error in programmin part of UpdateFPGA!!\n");
  CRC = getCRC(fwbuf, 512 * 1024);
  memset(fwbuf, 1024 * 1024, 00);
  printf("Verifying 512 kB block at 0x80000.. Source CRC=0x%02x\n", CRC);
  for (int page64 = 0; page64 < 8; page64++)
    DumpFW(0x80000 + page64 * 64 * 1024, 64);
