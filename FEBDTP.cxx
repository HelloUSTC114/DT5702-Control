...
int FEBDTP::Send_pkt(FEBDTP_PKT *pkt, int len, int timeout_us)
{
  ...
  while(numbytes>=0)
  {
    ...
    if(numbytes<0 && numpacks==0){
      if(Verbose>0) printf("No reply within %ld us!\n",tv.tv_usec);
      return 0;
    }
    if(numbytes<0) return retnumbytes;
    if(gpkt.src_mac[5] != thisdstmac[5] && thisdstmac[5] !=0xff){
      if(Verbose>0) printf("****************  Packet received from wrong FEB: 0x%02x (must be 0x%02x) Skipping..\n",gpkt.src_mac[5], thisdstmac[5]);
      continue;
    } // NOT from our FEB
    ...
    //else  
    if(Verbose>0) printf("<< ");
    if(Verbose>0) Print_gpkt(numbytes);
